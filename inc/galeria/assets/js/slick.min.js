...
jQuery(function($){
	...
	$('.is_slider').each(function(){
		var cfg = {"dots": true,"infinite": true,"speed": 1000, "autoplay":true, "autoplaySpeed": 5000, "slidesToShow": 4};
		$(this).slick( cfg );
	});

	$( '#main-slide img').imgLiquid();
	$(document).on('click','.each-galeria-brasa .navegacao-slider a',function(e){
		e.preventDefault();
		var $el = $( this );
		$('.each-galeria-brasa .loader').fadeIn()
		$('#main-slide img').addClass('blur');
		url = $(this).attr('data-src');
		$('#main-slide img').attr('src', url);
		$('#main-slide img').attr('data-src', url );
		$('#main-slide img[src="'+url+'"]').on('load',function(e){
			if ( '' ==  $el.find( 'img' ).attr( 'data-caption' ) ) {
				$( '#main-slide .each-image-title' ).html( '' );
				//alert( 'vazio' );
			} else {
				$( '#main-slide .each-image-title' ).html( $el.find( 'img' ).attr( 'data-caption' ) );
			}
			if ( '' ==  $el.find( 'img' ).attr( 'data-author' ) ) {
				$( '#main-slide .each-image-title-author' ).html( '' );
				//alert( 'vazio' );
			} else {
				$( '#main-slide .each-image-title-author' ).html( '<i class="fas fa-camera hidden-sm hidden-xs"></i> ' + $el.find( 'img' ).attr( 'data-author' ) );
			}
			$( '#main-slide .each-image-title' ).html( );
			$( '#main-slide img').imgLiquid();
			$('.each-galeria-brasa .loader').fadeOut();
			$('#main-slide img').removeClass('blur');
		});
	});
});
